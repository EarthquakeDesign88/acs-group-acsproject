...
<body>
            ...
            <div class="row mb-2">
                <div class="col-7">
                    <h3 class="card-title text-primary">
                        <?= $lang['categories']['title_header'] ?>
                        <?php $totalCategory = $categoryObj->totalRowCount(); ?>
                        <span class="badge bg-label-info"><?= $totalCategory?></span>
                    </h3>
                </div>
                <div class="col-5">
                    <a href="./create-category.php"><button type="button" class="btn btn-primary float-end"><?= $lang['categories']['btn']['new_category'] ?></button></a>
                </div>
            </div>
                    ...
                    <table class="table" id="categoryTable">
                        <thead class="th-resp">
                            <tr>
                                ...
                                <th><?= $lang['categories']['table']['category_name_en'] ?></th>
                        ...
                        <tbody>
                            ...
                            foreach ($categories as $category) {
                                $num++; ?>
                                <tr>
                                    <td data-label="<?= $lang['categories']['table']['no_col'] ?>"><i class="fab fa-angular fa-lg text-danger me-3"></i> <strong><?= $num ?></strong></td>
                                    <td data-label="<?= $lang['categories']['table']['category_name_th'] ?>"><?= $category['pcategory_name_th'] ?></td>
                                    <td data-label="<?= $lang['categories']['table']['category_name_en'] ?>"><?= $category['pcategory_name_en'] ?></td>
                                    <td data-label="<?= $lang['categories']['table']['action_col'] ?>">
                                        <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                            <div class="btn-group" role="group" aria-label="First group">
                                            <?php      
                                                if($_SESSION['user_role'] == 'admin') { ?> 
                                                    <a href="edit-category.php?pcategory_id=<?= $category['pcategory_id'] ?>" class="btn btn-outline-warning editData" title="<?= $lang['categories']['title']['edit'] ?>"><i class='bx bx-edit-alt'></i></a>
                                                    <button type="button" name="deleteData" id="<?= $category['pcategory_id'] ?>" class="btn btn-outline-danger deleteData" title="<?= $lang['categories']['title']['delete'] ?>"><i class='bx bxs-trash'></i></button>
                                            <?php  }?>
                                            </div>
                                        </div>
                                    </td>
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.deleteData', function(e){
            ...
            Swal.fire({
                icon: 'warning',
                title: '<?= $lang['alert']['cancel_title']['category']; ?>',
                text: '<?= $lang['alert']['cancel_text']['category']; ?>',
                ...
                if(result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: "./Controllers/backEnd/CategoryController/CategoryController-delete.php",
