...
<body>
            ...
            <div class="row mb-2">
                <div class="col-7">
                    <h3 class="card-title text-primary">
                        <?= $lang['owners']['title_header'] ?>
                        <?php $totalOwner = $ownerObj->totalRowCount(); ?>
                        <span class="badge bg-label-info"><?= $totalOwner?></span>
                    </h3>
                </div>
                <div class="col-5">
                    <a href="./create-owner.php"><button type="button" class="btn btn-primary float-end"><?= $lang['owners']['btn']['new_owner'] ?></button></a>
                </div>
            </div>
                    ...
                    <table class="table" id="ownerTable">
                        <thead class="th-resp">
                            <tr>
                                ...
                                <th><?= $lang['owners']['table']['owner_name_en_col'] ?></th>
                        ...
                        <tbody>
                            ...
                            foreach ($owners as $owner) {
                                $num++; ?>
                                <tr>
                                    <td data-label="<?= $lang['owners']['table']['no_col'] ?>"><i class="fab fa-angular fa-lg text-danger me-3"></i> <strong><?= $num ?></strong></td>
                                    <td data-label="<?= $lang['owners']['table']['owner_name_th_col'] ?>"><span class="ellipsis-tb"><?= $owner['owner_name_th'] ?></span></td>
                                    <td data-label="<?= $lang['owners']['table']['owner_name_en_col'] ?>"><span class="ellipsis-tb"><?= $owner['owner_name_en'] ?></span></td>
                                    <td data-label="<?= $lang['owners']['table']['action_col'] ?>">
                                        <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                            <div class="btn-group" role="group" aria-label="First group">
                                                <a href="./edit-owner.php?owner_id=<?= $owner['owner_id'] ?>" class="btn btn-outline-warning" title="<?= $lang['owners']['title']['edit'] ?>"><i class='bx bx-edit-alt'></i></a>
                                                <?php
                                                    if($_SESSION['user_role'] == 'admin') { ?>
                                                        <button type="button" name="deleteData" id="<?= $owner['owner_id'] ?>" class="btn btn-outline-danger deleteData" title="<?= $lang['owners']['title']['delete'] ?>"><i class='bx bxs-trash'></i></button>
                                                 <?php }
                                                ?>                                  
                                            </div>
                                        </div>
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.deleteData', function(e){
            ...
            Swal.fire({
                icon: 'warning',
                title: '<?= $lang['alert']['cancel_title']['owner']; ?>',
                text: '<?= $lang['alert']['cancel_text']['owner']; ?>',
                ...
                if(result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: "./Controllers/backEnd/OwnerController/OwnerController-delete.php",
