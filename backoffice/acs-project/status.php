...
<body>
            ...
            <div class="row mb-2">
                <div class="col-6">
                    <h3 class="card-title text-primary">
                        <?= $lang['status']['title_header']?>
                        <?php $totalStatus = $statusObj->totalRowCount(); ?>
                        <span class="badge bg-label-info"><?= $totalStatus ?></span>
                    </h3>
                </div>
                <div class="col-6">
                    <a href="./create-status.php" class="btn btn-primary float-end"><?= $lang['status']['btn']['new_status']?></a>
                </div>
            </div>
                    ...
                    <table class="table" id="statusTable">
                        <thead class="th-resp">
                            <tr>
                                ...
                                <th><?= $lang['status']['table']['status_name_en']?></th>
                        ...
                        <tbody>
                            ...
                            foreach ($statuss as $status) {
                                $num++; ?>
                                <tr>
                                    <td data-label="<?= $lang['status']['table']['no_col']?>"><i class="fab fa-angular fa-lg text-danger me-3"></i> <strong><?= $num ?></strong></td>
                                    <td data-label="<?= $lang['status']['table']['status_name_th']?>"><?= $status['status_name_th'] ?></td>
                                    <td data-label="<?= $lang['status']['table']['status_name_en']?>"><?= $status['status_name_en'] ?></td>
                                    <td data-label="<?= $lang['status']['table']['action_col']?>">
                                        <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                            <div class="btn-group" role="group" aria-label="First group">
                                                <a href="./edit-status.php?status_id=<?= $status['status_id'] ?>"><button type="button" class="btn btn-outline-warning" title="<?= $lang['status']['title']['edit']?>"><i class='bx bx-edit-alt'></i></button></a>
                                                <button type="button" name="deleteData" id="<?= $status['status_id'] ?>" class="btn btn-outline-danger deleteData" title="<?= $lang['status']['title']['delete']?>"><i class='bx bxs-trash'></i></button>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.deleteData', function(e){
            ...
            Swal.fire({
                icon: 'warning',
                title: '<?= $lang['alert']['cancel_title']['status']; ?>',
                text: '<?= $lang['alert']['cancel_text']['status']; ?>',
                ...
                if(result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: "./Controllers/backEnd/StatusController/StatusController-delete.php",
