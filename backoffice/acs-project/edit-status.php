        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            <div class="row">
                <div class="col-lg-12 mb-4 order-0">
                    <div class="row mb-2">
                        <div class="col-6">
                            <h3 class="card-title text-primary"><?= $lang['status']['edit_status_header']?> <span id="load-data"></span></h3>
                        </div>
                        <div class="col-6">
                            ...
                            <form id="updateStatusForm">
                                <div class="row">
                                    <?php
                                        ...
                                        $statuss =  $statusObj->getStatusById($status_id);

                                        foreach ($statuss as $status) { ?>
                                            <div class="mb-3 col-md-6">
                                                <label for="StatusNameTH" class="form-label"><?= $lang['status']['status_name_th'] ?></label>
                                                <input type="text" class="form-control" id="status_name_th" name="status_name_th" placeholder="<?= $lang['status']['placeholder']['status_name_th']?>" value="<?= $status['status_name_th']?>" autocomplete="off"/>
                                            </div>
                                            <div class="mb-3 col-md-6"></div>
                                            <div class="mb-3 col-md-6">
                                                <label for="StatusNameEN" class="form-label"><?= $lang['status']['status_name_en'] ?></label>
                                                <div class="input-group input-group-merge">
                                                    <input type="text" class="form-control" id="status_name_en" name="status_name_en" placeholder="<?= $lang['status']['placeholder']['status_name_en']?>" value="<?= $status['status_name_en']?>" autocomplete="off"/>
                                                </div>
                                            </div>
    ...
    <script type="text/javascript">
        $('#updateStatusForm').on("submit", function(e) {
            ...
            if ($('#status_name_th').val() == '') {
                ...
                var updateStatusForm = $(this).serialize();

                $.ajax({
                    type: "POST",
                    url: "./Controllers/backEnd/StatusController/StatusController-update.php",
                    ...
                    success: function(response) {
                        if (response['status'] == 'success') {
                            ...
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    window.location = "edit-status.php";
                                }
                            })
                        }
                    }
