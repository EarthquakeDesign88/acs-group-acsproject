...
<body>
            ...
            <div class="row mb-2">
                <div class="col-6">
                    <h3 class="card-title text-primary">
                        <?= $lang['departments']['title_header'] ?>
                        <?php $totalDepartment = $departmentObj->totalRowCount(); ?>
                        <span class="badge bg-label-info"><?= $totalDepartment?></span>
                    </h3>
                </div>
                <div class="col-6">
                    <a href="./create-department.php"><button type="button" class="btn btn-primary float-end"><?= $lang['departments']['btn']['new_department'] ?></button></a>
                </div>
            </div>
                    ...
                    <table class="table" id="departmentTable">
                        <thead>
                            <tr>
                                ...
                                <th><?= $lang['departments']['table']['department_desc_col'] ?></th>
                        ...
                        <tbody class="table-border-bottom-0">
                            ...
                            foreach ($departments as $department) {
                                $num++; ?>
                                <tr>
                                    <td data-label="<?= $lang['departments']['table']['no_col'] ?>"><i class="fab fa-angular fa-lg text-danger me-3"></i> <strong><?= $num ?></strong></td>
                                    <td data-label="<?= $lang['departments']['table']['department_name_col'] ?>"><span class="ellipsis-tbs"><?= $department['department_name'] ?></span></td>
                                    <td data-label="<?= $lang['departments']['table']['department_desc_col'] ?>"><span class="ellipsis-tbs"><?= $department['department_desc'] ?></span></td>
                                    <td data-label="<?= $lang['departments']['table']['action_col'] ?>">
                                        <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                            <div class="btn-group" role="group" aria-label="First group">
                                                <a href="edit-department.php?department_id=<?= $department['department_id'] ?>" class="btn btn-outline-warning editData" title="<?= $lang['departments']['title']['edit'] ?>"><i class='bx bx-edit-alt'></i></a>
                                                <button type="button" name="deleteData" id="<?= $department['department_id'] ?>" class="btn btn-outline-danger deleteData" title="<?= $lang['departments']['title']['delete'] ?>"><i class='bx bxs-trash'></i></button>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.deleteData', function(e){
            ...
            Swal.fire({
                icon: 'warning',
                title: '<?= $lang['alert']['cancel_title']['department']; ?>',
                text: '<?= $lang['alert']['cancel_text']['department']; ?>',
                ...
                if(result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: "./Controllers/backEnd/DepartmentController/DepartmentController-delete.php",
